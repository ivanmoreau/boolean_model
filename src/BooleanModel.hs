...
import Data.Map (Map, fromList, insert, insertWith, (!), delete)
import qualified Data.Map as Dm
import Data.Text.IO as DT (readFile)


rone :: [Text]
rone = ["$user tweeted ", " $ht", " $user"]
...
getWords l = fromList (map (\x -> (x, fromList [(0, False)])) (Data.List.delete "" (nub (splitOn " " (replace "\n" " " l)))) )
...
aa m t i = let r = m ! t in
  insert t (insert i True r) m

--mLine :: 
byLine :: [Text] -> Map Text (Map Int Bool) -> Int -> Map Text (Map Int Bool)
byLine [] m _ = m
byLine (l:ls) m c = byLine ls (foldr (\x y -> aa y x c) m (nub (splitOn " " l))) (c + 1)

genMatrix :: Text
  -> Map Text (Map Int Bool)
genMatrix t = let ff = fixWords t rone 
                  gw = getWords ff
                  ts = (splitOn "\n" ff)  
                  em = emptymatrix gw (length ts) in
  byLine ts em 0 
